...
| **👆 動画再生中の画面タップ・操作性の完全復旧** | Android WebView の `builtInZoomControls` によるタッチイベント横取りを解除。<br>プレビュー再生時のミュート解除オーバーレイ（`.ytp-unmute` / `.preview-mute-button`）を自動検知して解放し、字幕ウィンドウのタッチ透過（`pointer-events: none`）を適用。動画タップでの操作バー表示・一時停止・CC・シーク操作がスムーズに反応するよう修正。<br>モバイル版Chrome最新UAを適用し、YouTubeの標準コントロールオーバーレイを解放。 |
...
| **🖥️ YouTube動画の全画面（最大化）再生対応** | YouTubeプレーヤー右下の最大化ボタン（`.fullscreen-icon`）が押せない・反応しない問題を完全解消。<br>WebViewのHTML5 Videoネイティブ全画面API（`requestFullscreen` / `webkitRequestFullscreen`）フックおよびキャプチャフェーズのタップインターセプターを実装し、YouTubeプレーヤー内の最大化ボタンおよびトップバーの全画面ボタン双方から確実に横画面（ランドスケープ）全画面へ切り替え可能に。<br>`WebChromeClient.onShowCustomView` / `onHideCustomView` を完全実装し、最大化時もリアルタイム日本語字幕をフローティングオーバーレイ表示して快適視聴可能。Android「戻る」ボタンでの全画面解除にも完全対応。 |
| **🎛️ 字幕オーバーレイの退避・最小化・自由ドラッグ移動対応** | YouTubeの設定（再生速度や画質などのボトムシートメニュー）を開いた際に「字幕待機中」カードが重なって操作できない問題を完全解消。<br>・**上下位置切り替え（`⇅`）**: ワンタップでカードが動画直下へ瞬時に移動し、下部設定領域を全開放。<br>・**自由ドラッグ移動**: 上部のドラッグハンドル（`─`）をつかんで画面内の好きな位置へ自由に配置可能。<br>・**ワンタップ最小化（`▼` / `▲`）**: 字幕内容を折りたたみ、極小の1行バーへコンパクト化。<br>・**非表示（`✕`）＆ 再表示ピル**: カードを画面から消去でき、右下のフローティングボタン `[ 💬 字幕を表示 ]` でいつでも復帰。<br>・**ガイダンス通知の非表示**: 「⚠️ 字幕(CC)がオフです」の警告通知に「閉じる」ボタンを追加し、不要な時はすぐに消去可能。 |
...
YouTubeで再生速度の変更や画質調整、コメント欄などのボトムシートメニューを開いた際、画面下部の字幕カードが重なってタッチ操作を妨げることがないよう、多彩な回避・操作機能を搭載しています：
- **上下位置切り替え (`⇅`)**: ヘッダーの切り替えボタンを押すと、カードが動画のすぐ下（Y座標約260dp）へスムーズにアニメーション移動。下部の設定メニュー領域を一発で全開にします。
- **自由ドラッグ移動 (`─` ドラッグハンドル)**: カード最上部のドラッグバーを指でなぞることで、画面内の見やすい場所へ自由に移動・配置できます（画面外へのはみ出し防止クランプ付き）。
- **ワンタップ最小化 (`▼` / `▲`)**: 最小化ボタンで字幕テキスト領域を瞬時に折りたたみ、約36dpの極小バーに変形させます。
- **非表示 (`✕`) & 再表示フローティングボタン (`[ 💬 字幕を表示 ]`)**: 閉じるボタンでカードを完全に隠蔽。画面右下に半透明の `[ 💬 字幕を表示 ]` ピルが出現し、ワンタップで元の位置に復元できます。
- **警告ガイダンスの「閉じる」対応**: 「字幕(CC)がオフです」等の黄色い通知バナーも、「閉じる」をタップして即座に非表示にできます。
