...
| **🖥️ YouTube動画の全画面（最大化）再生対応** | YouTubeプレーヤー右下の最大化ボタンが押せない・反応しない問題を解消。<br>Android WebViewの `WebChromeClient.onShowCustomView` / `onHideCustomView` を完全実装し、HTML5 Fullscreen APIを有効化。最大化時もリアルタイム日本語字幕をフローティングオーバーレイ表示して快適視聴可能。トップバーにも専用の全画面トグルボタンを追加。 |
