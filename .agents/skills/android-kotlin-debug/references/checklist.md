...
## J. YouTube PO Token・ネットワーク透過インターセプトチェック

- [ ] 字幕トラック取得時に外部からの独立 `fetch()` に依存せず、公式プレイヤーの内部通信（`fetch`/`XHR` フック）から透過捕捉しているか？
- [ ] 字幕未捕捉時に公式プレイヤーの字幕モジュール（`loadModule("captions")`）やCCボタンを自動タップして正規通信を発行させる能動的トリガーを備えているか？
- [ ] モバイルYouTubeの相対URL（`/api/timedtext?...`）を `https://m.youtube.com/...` に自動補正しているか？

---

## K. 非言語音響マーカー＆無音・BGM動画ハンドリングチェック

- [ ] `sanitizeCaption` で `[Music]`, `[Applause]`, `[Laughter]`, `[音楽]`, `[拍手]`, `♪` 等の非言語音響タグを確実に除去しているか？
- [ ] 音響タグ除去後に実テキストが空（または極小）になったセグメントを、発話とみなさず安全にスキップ・除外しているか？
- [ ] 全編がBGM・タイムラプス・無言動画の場合に、「有効な発話音声が含まれていません」とユーザーへ親切に案内し、安全に停止できているか？

---

## L. キャッシュ整合性・破損ファイル破棄チェック

- [ ] ダウンロードされたメディアファイルが数KBのHTMLエラーページ（`<!DOCTYPE html>`）でないか検証しているか？
- [ ] 破損ファイルや空ファイルを検知した場合、即座に削除（破棄）して次回の解析クラッシュを防止しているか？
